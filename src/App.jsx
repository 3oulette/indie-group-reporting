...
function calcNbrJours(dateOuv,dateFerm){if(!dateOuv||!dateFerm)return 0;const a=new Date(dateOuv),b=new Date(dateFerm);if(isNaN(a)||isNaN(b)||b<a)return 0;return Math.round((b-a)/(86400000))+1;}

// ── LOYER/IG CALC ──
// Types: "fixe" = montant annuel / nbrJours * nJ
//        "variable" = % du CA
//        "fixe_variable" = fixe + variable
function calcCharge(type,annuel,pctCA,nbrJoursSaison,nJPeriode,ca){
  let fixe=0, variable=0;
  if(type==="fixe"||type==="fixe_variable"){fixe=nbrJoursSaison>0?Math.round(annuel/nbrJoursSaison*nJPeriode):0;}
  if(type==="variable"||type==="fixe_variable"){variable=Math.round(ca*(pctCA||0));}
  return fixe+variable;
}

// ── DATA ──
const INIT_ESTABLISHMENTS = [
  { id:"pablo", name:"Pablo ST", group:"ete", coefLiquide:0.27, coefSolide:0.24, coefLiquideN1:0.27, coefSolideN1:0.24, coefLiquideBudget:0.25, coefSolideBudget:0.23, loyerAnnuel:133337, loyerType:"fixe_variable", loyerPctCA:0.05, igAnnuel:600000, igType:"variable", igPctCA:0.12, autresChargesAnnuel:1166844, dateOuv:"2026-04-06", dateFerm:"2026-10-15" },
  { id:"indie_beach", name:"Indie Beach", group:"ete", coefLiquide:0.19, coefSolide:0.19, coefLiquideN1:0.18, coefSolideN1:0.19, coefLiquideBudget:0.18, coefSolideBudget:0.18, loyerAnnuel:162279, loyerType:"fixe", loyerPctCA:0, igAnnuel:720000, igType:"variable", igPctCA:0.10, autresChargesAnnuel:2405296, dateOuv:"2026-04-18", dateFerm:"2026-10-04" },
  { id:"playamigos", name:"Playamigos", group:"ete", coefLiquide:0.17, coefSolide:0.17, coefLiquideN1:0.17, coefSolideN1:0.17, coefLiquideBudget:0.16, coefSolideBudget:0.16, loyerAnnuel:162279, loyerType:"fixe", loyerPctCA:0, igAnnuel:154299, igType:"fixe", igPctCA:0, autresChargesAnnuel:571181, dateOuv:"2026-04-10", dateFerm:"2026-10-05" },
  { id:"cherry", name:"Cherry", group:"ete", coefLiquide:0.24, coefSolide:0.24, coefLiquideN1:0.24, coefSolideN1:0.24, coefLiquideBudget:0.23, coefSolideBudget:0.23, loyerAnnuel:148000, loyerType:"fixe", loyerPctCA:0, igAnnuel:126840, igType:"fixe", igPctCA:0, autresChargesAnnuel:324703, dateOuv:"2026-04-15", dateFerm:"2026-10-12" },
  { id:"sauvageonne", name:"La Sauvageonne", group:"ete", coefLiquide:0.22, coefSolide:0.22, coefLiquideN1:0.21, coefSolideN1:0.21, coefLiquideBudget:0.21, coefSolideBudget:0.21, loyerAnnuel:40000, loyerType:"fixe", loyerPctCA:0, igAnnuel:154560, igType:"variable", igPctCA:0.08, autresChargesAnnuel:361693, dateOuv:"2026-04-10", dateFerm:"2026-10-05" },
  { id:"ormeau", name:"Ormeau", group:"ete", coefLiquide:0.23, coefSolide:0.23, coefLiquideN1:0.25, coefSolideN1:0.25, coefLiquideBudget:0.23, coefSolideBudget:0.23, loyerAnnuel:234315, loyerType:"fixe", loyerPctCA:0, igAnnuel:71329, igType:"fixe", igPctCA:0, autresChargesAnnuel:239061, dateOuv:"2026-04-01", dateFerm:"2026-10-31" },
  { id:"cherry_paris", name:"Cherry Paris", group:"ete", coefLiquide:0.22, coefSolide:0.22, coefLiquideN1:0.22, coefSolideN1:0.22, coefLiquideBudget:0.21, coefSolideBudget:0.21, loyerAnnuel:85000, loyerType:"fixe", loyerPctCA:0, igAnnuel:90000, igType:"fixe", igPctCA:0, autresChargesAnnuel:180000, dateOuv:"2026-05-01", dateFerm:"2026-09-30" },
  { id:"pablo_sbh", name:"Pablo SBH", group:"hiver", coefLiquide:0.20, coefSolide:0.20, coefLiquideN1:0.20, coefSolideN1:0.20, coefLiquideBudget:0.19, coefSolideBudget:0.19, loyerAnnuel:0, loyerType:"fixe", loyerPctCA:0, igAnnuel:0, igType:"fixe", igPctCA:0, autresChargesAnnuel:450000, dateOuv:"2025-11-01", dateFerm:"2026-04-30" },
  { id:"sauva_megeve", name:"Sauva Megeve", group:"hiver", coefLiquide:0.25, coefSolide:0.25, coefLiquideN1:0.25, coefSolideN1:0.25, coefLiquideBudget:0.24, coefSolideBudget:0.24, loyerAnnuel:120000, loyerType:"fixe", loyerPctCA:0, igAnnuel:80000, igType:"fixe", igPctCA:0, autresChargesAnnuel:280000, dateOuv:"2025-12-10", dateFerm:"2026-03-30" },
  { id:"cat_club", name:"Cat Club", group:"hiver", coefLiquide:0.30, coefSolide:0.30, coefLiquideN1:0.30, coefSolideN1:0.30, coefLiquideBudget:0.28, coefSolideBudget:0.28, loyerAnnuel:95000, loyerType:"fixe", loyerPctCA:0, igAnnuel:60000, igType:"fixe", igPctCA:0, autresChargesAnnuel:200000, dateOuv:"2025-12-15", dateFerm:"2026-03-25" },
  { id:"cafe_flora", name:"Cafe Flora", group:"ete", coefLiquide:0.22, coefSolide:0.22, coefLiquideN1:0.22, coefSolideN1:0.22, coefLiquideBudget:0.21, coefSolideBudget:0.21, loyerAnnuel:0, loyerType:"fixe", loyerPctCA:0, igAnnuel:0, igType:"fixe", igPctCA:0, autresChargesAnnuel:150000, dateOuv:"2026-04-15", dateFerm:"2026-10-15" },
].map(e=>({...e,nbrJours:calcNbrJours(e.dateOuv,e.dateFerm)}));
...
function calcKPI(est,data,from,to){
  const nD=fdr(data.n,from,to),f1=sN1(from),t1=sN1(to),n1D=fdr(data.n1,f1,t1),bD=fdr(data.budget||[],f1,t1),pD=fdr(data.previCA||[],f1,t1);
  const ca=sm(nD,"ca"),caN1=sm(n1D,"ca"),caP=sm(pD,"previ")||sm(bD,"ca");
  const ms=sm(nD,"ms"),ex=sm(nD,"extra"),lo=sm(nD,"logement"),msT=ms+ex+lo;
  const msN1=sm(n1D,"ms")+sm(n1D,"extra")+sm(n1D,"logement");
  const djS=sm(nD,"dj")+sm(nD,"secu")+sm(nD,"light")+sm(nD,"hotel");
  const djN1=sm(n1D,"dj")+sm(n1D,"secu")+sm(n1D,"light")+sm(n1D,"hotel");
  const nJ=nD.length||1;const nJS=est.nbrJours||nJ;
  const ach=Math.round(ca*est.coefSolide),achN1=Math.round(caN1*est.coefSolideN1);
  const loy=calcCharge(est.loyerType,est.loyerAnnuel,est.loyerPctCA,nJS,nJ,ca);
  const loyN1=calcCharge(est.loyerType,est.loyerAnnuel,est.loyerPctCA,nJS,nJ,caN1);
  const ig=calcCharge(est.igType,est.igAnnuel,est.igPctCA,nJS,nJ,ca);
  const igN1=calcCharge(est.igType,est.igAnnuel,est.igPctCA,nJS,nJ,caN1);
  const aut=nJS>0?Math.round(est.autresChargesAnnuel/nJS*nJ):0;
  const ebitda=ca-msT-djS-ach-loy-ig-aut;const ebitdaN1=caN1-msN1-djN1-achN1-loyN1-igN1-aut;
  const msB=Math.round(caP*.22),achB=Math.round(caP*est.coefSolideBudget);
  const loyB=calcCharge(est.loyerType,est.loyerAnnuel,est.loyerPctCA,nJS,nJ,caP);
  const igB=calcCharge(est.igType,est.igAnnuel,est.igPctCA,nJS,nJ,caP);
  const ebitdaB=caP-msB-achB-loyB-igB-aut-Math.round(djN1*1.05);
  const cvt=sm(nD,"couverts"),cvtN1=sm(n1D,"couverts");const tm=cvt?ca/cvt:0,tmN1=cvtN1?caN1/cvtN1:0;const daily=nD.slice(-14).map(d=>d.ca);
  return{ca,caN1,caP,ms:msT,msRaw:ms,ex,lo,msN1,msPct:ca?msT/ca:0,msPctN1:caN1?msN1/caN1:0,djS,djN1,ach,achN1,loy,loyN1,loyB,ig,igN1,igB,aut,ebitda,ebitdaN1,ebitdaB,msB,achB,cvt,cvtN1,tm:Math.round(tm),tmN1:Math.round(tmN1),daily,nJ};
}
...
// ── DASHBOARD CARD ──
function EstCard({est,kpis:k,onClick}){const em=k.ca?k.ebitda/k.ca:0;return(<GC onClick={onClick} style={{display:"flex",flexDirection:"column",gap:14}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}><div><div style={{fontSize:16,fontWeight:600,color:C.tx}}>{est.name}</div><div style={{fontSize:11,color:C.mu,marginTop:2}}>{est.group==="ete"?"Saison ete":"Saison hiver"} | {est.nbrJours}j</div></div><Badge value={pv(k.ca,k.caN1)} big/></div><div><div style={{fontSize:11,color:C.dm,marginBottom:4}}>CA</div><div style={{display:"flex",alignItems:"baseline",gap:8}}><span style={{fontSize:26,fontWeight:700,fontVariantNumeric:"tabular-nums",color:C.tx}}>{fmtK(k.ca)}</span><span style={{fontSize:12,color:C.mu}}>N-1: {fmtK(k.caN1)}</span></div></div><Spark data={k.daily}/><div style={{display:"flex",gap:16}}><div style={{flex:1}}><div style={{fontSize:11,color:C.dm}}>MS</div><div style={{fontSize:15,fontWeight:600}}>{fmtPct(k.msPct)}</div><div style={{fontSize:10,color:C.mu}}>N-1: {fmtPct(k.msPctN1)}</div></div><div style={{flex:1}}><div style={{fontSize:11,color:C.dm}}>EBITDA</div><div style={{fontSize:15,fontWeight:600,color:k.ebitda<0?C.rd:C.gn}}>{fmtK(k.ebitda)}</div><div style={{fontSize:10,color:C.mu}}>{fmtPct(em)} marge</div></div><div style={{flex:1}}><div style={{fontSize:11,color:C.dm}}>Couverts</div><div style={{fontSize:15,fontWeight:600}}>{fmt(k.cvt)}</div><div style={{fontSize:10,color:C.mu}}>TM: {fmt(k.tm)}E</div></div></div><div style={{textAlign:"right"}}><span style={{fontSize:11,color:C.ac,fontWeight:500}}>Voir le detail &rarr;</span></div></GC>);}
...
{tab==="coefs"&&<GC hover={false}><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}><thead><tr style={{borderBottom:`1px solid ${C.gb}`,color:C.mu,fontSize:10,textTransform:"uppercase"}}><th style={{textAlign:"left",padding:"8px 12px"}}>Etablissement</th><th style={{textAlign:"center",padding:"8px 12px"}}>Liq. N</th><th style={{textAlign:"center",padding:"8px 12px"}}>Sol. N</th><th style={{textAlign:"center",padding:"8px 12px"}}>Liq. N-1</th><th style={{textAlign:"center",padding:"8px 12px"}}>Sol. N-1</th><th style={{textAlign:"center",padding:"8px 12px"}}>Liq. Budget</th><th style={{textAlign:"center",padding:"8px 12px"}}>Sol. Budget</th><th style={{padding:"8px 12px"}}></th></tr></thead><tbody>{ests.map(est=>(<tr key={est.id} style={{borderBottom:`1px solid ${C.gb}22`}}><td style={{padding:"10px 12px",fontWeight:500}}>{est.name}</td>{["coefLiquide","coefSolide","coefLiquideN1","coefSolideN1","coefLiquideBudget","coefSolideBudget"].map(f=>(<td key={f} style={{padding:"10px 12px",textAlign:"center"}}><span style={{padding:"4px 10px",borderRadius:6,background:C.bg,border:`1px solid ${C.gb}`,display:"inline-block",minWidth:55,fontSize:12}}>{fmtPct(est[f])}</span></td>))}<td style={{padding:"10px 12px",textAlign:"center"}}><Btn small onClick={()=>{setEd({...est});setModal("coef");}}>Edit</Btn></td></tr>))}</tbody></table></div></GC>}

{/* FIX #1+#2: Modals don't close on backdrop, save works properly */}
{modal==="user"&&<Modal title={ed._new?"Nouvel utilisateur":"Modifier "+ed.name} onClose={()=>setModal(null)}><div style={{display:"grid",gap:14}}><Fld label="Login"><Inp value={ed.login||""} onChange={e=>{const v=e.target.value;setEd(d=>({...d,login:v}));}}/></Fld><Fld label="Nom"><Inp value={ed.name||""} onChange={e=>{const v=e.target.value;setEd(d=>({...d,name:v}));}}/></Fld><Fld label="Mot de passe"><Inp type="password" value={ed.password||""} onChange={e=>{const v=e.target.value;setEd(d=>({...d,password:v}));}}/></Fld><Fld label="Role"><Sel value={ed.role||"viewer"} onChange={e=>{const v=e.target.value;setEd(d=>({...d,role:v}));}}><option value="admin">Admin</option><option value="viewer">Viewer</option></Sel></Fld><Fld label="Etablissements"><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{ests.map(est=>{const sel=(ed.establishments||[]).includes(est.id);return<button key={est.id} onClick={()=>setEd(d=>({...d,establishments:sel?d.establishments.filter(x=>x!==est.id):[...(d.establishments||[]),est.id]}))} style={{padding:"4px 10px",borderRadius:6,border:`1px solid ${sel?C.ac:C.gb}`,background:sel?C.ad:"transparent",color:sel?C.ac:C.dm,fontSize:11,cursor:"pointer",fontFamily:"inherit"}}>{est.name}</button>;})}</div></Fld><Btn primary onClick={saveUser}>Enregistrer</Btn></div></Modal>}

{/* FIX #3: loyerType and igType now support fixe, variable, fixe_variable */}
{modal==="est"&&<Modal title={"Modifier "+ed.name} onClose={()=>setModal(null)} wide><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
<Fld label="Date ouverture"><Inp type="date" value={ed.dateOuv||""} onChange={e=>{const v=e.target.value;setEd(d=>{const nd={...d,dateOuv:v};nd.nbrJours=calcNbrJours(nd.dateOuv,nd.dateFerm);return nd;});}}/></Fld>
<Fld label="Date fermeture"><Inp type="date" value={ed.dateFerm||""} onChange={e=>{const v=e.target.value;setEd(d=>{const nd={...d,dateFerm:v};nd.nbrJours=calcNbrJours(nd.dateOuv,nd.dateFerm);return nd;});}}/></Fld>
<div style={{gridColumn:"1/-1",fontSize:12,color:C.dm,background:C.gl,padding:"8px 12px",borderRadius:8}}>Jours de saison: <strong style={{color:C.ac}}>{ed.nbrJours||0}</strong> (calcul auto)</div>
<Fld label="Type loyer"><Sel value={ed.loyerType||"fixe"} onChange={e=>{const v=e.target.value;setEd(d=>({...d,loyerType:v}));}}><option value="fixe">Fixe</option><option value="variable">Variable (% CA)</option><option value="fixe_variable">Fixe + Variable</option></Sel></Fld>
{(ed.loyerType==="fixe"||ed.loyerType==="fixe_variable")&&<Fld label="Loyer annuel fixe"><Inp type="number" value={ed.loyerAnnuel||0} onChange={e=>{const v=+e.target.value;setEd(d=>({...d,loyerAnnuel:v}));}}/></Fld>}
{(ed.loyerType==="variable"||ed.loyerType==="fixe_variable")&&<Fld label="Loyer % CA"><Inp type="number" step="0.001" value={ed.loyerPctCA||0} onChange={e=>{const v=+e.target.value;setEd(d=>({...d,loyerPctCA:v}));}}/></Fld>}
<Fld label="Type IG"><Sel value={ed.igType||"fixe"} onChange={e=>{const v=e.target.value;setEd(d=>({...d,igType:v}));}}><option value="fixe">Fixe</option><option value="variable">Variable (% CA)</option><option value="fixe_variable">Fixe + Variable</option></Sel></Fld>
{(ed.igType==="fixe"||ed.igType==="fixe_variable")&&<Fld label="IG annuel fixe"><Inp type="number" value={ed.igAnnuel||0} onChange={e=>{const v=+e.target.value;setEd(d=>({...d,igAnnuel:v}));}}/></Fld>}
{(ed.igType==="variable"||ed.igType==="fixe_variable")&&<Fld label="IG % CA"><Inp type="number" step="0.001" value={ed.igPctCA||0} onChange={e=>{const v=+e.target.value;setEd(d=>({...d,igPctCA:v}));}}/></Fld>}
<Fld label="Autres charges annuel"><Inp type="number" value={ed.autresChargesAnnuel||0} onChange={e=>{const v=+e.target.value;setEd(d=>({...d,autresChargesAnnuel:v}));}}/></Fld>
</div><Btn primary onClick={saveEst} style={{marginTop:20}}>Enregistrer</Btn></Modal>}

{modal==="coef"&&<Modal title={"Coefs MP - "+ed.name} onClose={()=>setModal(null)}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>{[["coefLiquide","Liquide N"],["coefSolide","Solide N"],["coefLiquideN1","Liquide N-1"],["coefSolideN1","Solide N-1"],["coefLiquideBudget","Liquide Budget"],["coefSolideBudget","Solide Budget"]].map(([k,l])=>(<Fld key={k} label={l}><Inp type="number" step="0.01" value={ed[k]||0} onChange={e=>{const v=+e.target.value;setEd(d=>({...d,[k]:v}));}}/></Fld>))}</div><Btn primary onClick={saveCoef} style={{marginTop:20}}>Enregistrer</Btn></Modal>}
...
export default function App(){
  ...
  const[ests,setEsts]=useState(INIT_ESTABLISHMENTS);
  const[usrs,setUsrs]=useState(INIT_USERS);
  const[cm,setCm]=useState("periode");
  const today=new Date().toISOString().split("T")[0];
  const[wS,wE]=wr(today);
  const[from,setFrom]=useState(wS);
  const[to,setTo]=useState(wE);
  const onR=useCallback((f,t)=>{setFrom(f);setTo(t);},[]);
  const demo=useMemo(()=>buildDemo(ests),[ests]);
  const uEsts=useMemo(()=>user?ests.filter(e=>user.establishments.includes(e.id)):[],[user,ests]);
  const allK=useMemo(()=>{const r={};uEsts.forEach(est=>{if(demo[est.id])r[est.id]=calcKPI(est,demo[est.id],from,to);});return r;},[uEsts,demo,from,to]);
  const gK=useMemo(()=>{let ca=0,caN1=0,caP=0,ms=0,msN1=0,ebitda=0,ebitdaN1=0;Object.values(allK).forEach(k=>{ca+=k.ca;caN1+=k.caN1;caP+=k.caP;ms+=k.ms;msN1+=k.msN1;ebitda+=k.ebitda;ebitdaN1+=k.ebitdaN1;});return{ca,caN1,caP,ms,msN1,ebitda,ebitdaN1};},[allK]);
  ...
  {view!=="admin"&&<DateBar from={from} to={to} onRange={onR} compMode={cm} setCompMode={setCm}/>}
  {view==="admin"&&<AdminConsole establishments={ests} setEstablishments={setEsts} users={usrs} setUsers={setUsrs} onBack={()=>setView("dashboard")}/>}
  {view==="dashboard"&&<><GroupBar kpis={gK}/><div style={{fontSize:11,color:C.mu,textTransform:"uppercase",letterSpacing:2,marginBottom:12}}>Etablissements ({cm==="periode"?"Periode selectionnee":cm==="cumule"?"Cumule depuis ouverture":"Atterrissage saison"})</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:14}}>{uEsts.map(est=>allK[est.id]&&<EstCard key={est.id} est={est} kpis={allK[est.id]} onClick={()=>{setSelEst(est);setView("detail");}}/>)}</div></>}
  {view==="detail"&&selEst&&allK[selEst.id]&&<DetailView est={selEst} kpis={allK[selEst.id]} onBack={()=>{setView("dashboard");setSelEst(null);}}/>}
  </div></div>);
}
